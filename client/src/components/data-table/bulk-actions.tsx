...
export function DataTableBulkActions<TData>({
  ...
  const entityWord = t(entityName)
  const entityWordPlural = `${entityWord}s`

  // Announce selection changes to screen readers
  useEffect(() => {
    if (selectedCount > 0) {
      const message = t('You selected {{n}} {{entity}}. Bulk actions toolbar is available.', {
        n: selectedCount,
        entity: selectedCount > 1 ? entityWordPlural : entityWord,
      })
  ...
  }, [selectedCount, entityName, entityWord, entityWordPlural, t])
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role='toolbar'
        aria-label={t('Bulk actions for {{n}} {{entity}}', {
          n: selectedCount,
          entity: selectedCount > 1 ? entityWordPlural : entityWord,
        })}
        ...
        >
          ...
          >
            <Badge
              ...
              aria-label={t('{{n}} selected', { n: selectedCount })}
            >
              {selectedCount}
            </Badge>{' '}
            <span className='hidden sm:inline'>
              {selectedCount > 1 ? entityWordPlural : entityWord}
            </span>{' '}
            {t('selected')}
          </div>
